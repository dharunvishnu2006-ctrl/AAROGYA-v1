...
with tab3:
    ...
    hospitals = [
        Hospital('H001', 'Apollo Chennai', 'Chennai', 500),
        Hospital('H002', 'AIIMS Delhi', 'Delhi', 1000),
        Hospital('H003', 'Fortis Mumbai', 'Mumbai', 300),
    ]
    for h in hospitals:
        ...
        st.caption(f'{rate}% occupied - {current} patients')
...
try:
    ...
    if risk == 'Low':
        ...
        st.warning(f'{row["name"]} - Medium Risk')
    else:
        st.error(f'{row["name"]} - High Risk')

except ValueError as e:
    st.error(f"⚠️ Validation Error for Patient {row['patient_id']} ({row['name']}): {e}")
    st.info("This record was flagged at the ingestion boundary and cannot be scored.")